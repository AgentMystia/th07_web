...
- **Stage 1–6 + Extra + Phantasm 均可进入并游玩**（难度 Easy–Lunatic /
  Extra / Phantasm）：道中、中 Boss、对话立绘、Boss 符卡与激光系统均由
  原作 ECL/ANM/STD 驱动；标题菜单含「Extra Start」入口。
- 关卡流程：Stage Clear 结算（Clear / Point / Graze / Cherry + 难度倍率；
  终关 All Clear! + Player/Bomb 行）→ `Z` 进下一关并携带
  score/lives/bombs/power/graze/cherry。
- 标题画面 → 难度选择 → 机体/符卡类型选择（灵梦/魔理沙/咲夜 × A/B），
  全部使用原作 `title01.anm` 菜单脚本与美术。
- 自机数据直接读取原作 `.sht`：移动速度、判定、擦弹、火力表、
  Deathbomb 窗口（灵梦 15 / 魔理沙 8 / 咲夜 6 帧）、30 帧射击周期。
...
- BGM 全曲目（`th07_01`–`th07_19` / `13b`）按 `thbgm.fmt` 采样级循环点
  无缝循环；音效为原作 WAV。

> 高还原度仍在推进中：部分后段 Boss 相位、Stage 4 多体姐妹编排、
> 个别弹幕 effect 与背景 bank 切换等见下方「已知差距」。欢迎反馈。
...
- 集中注册的近似项见 [AGENTS.md §7](AGENTS.md)（聚焦 Option 轨道速率、
  Bomb 演出、部分 HUD 摆位、Spell 声明演出、Boss 底部 “Enemy” 标记
  等）。
- Stage 4 多 slot 姐妹编排、部分后段 Boss death-callback、弹幕 effect
  id 17/18（种子弹引爆）与 7/8（激光反弹）、STD op 29 背景 bank 切换
  等仍未完全对齐原作。
- Extra / Phantasm 开局 Bomb / Power 取社区惯例（PROBABLE，未 exe 核对）。

### 支持一下（可选）

如果这个项目对你有帮助，又恰好想请我喝杯咖啡 / 续一点 API token，
可以往下面这个地址打一点 **USDT (ERC-20)**（任意 EVM 链均可）。完全自愿，
金额随意，不支持也不影响使用：
...
- **Stages 1–6 + Extra + Phantasm are enterable and playable**
  (Easy–Lunatic / Extra / Phantasm): midbosses, portrait dialogue, boss
  spellcards, and the laser system all driven by the original ECL/ANM/STD
  data. The title menu includes an Extra Start entry.
- Stage flow: Stage Clear tally (Clear / Point / Graze / Cherry + rank
  multiplier; route-final All Clear! with Player/Bomb rows) → `Z` advances
  to the next stage carrying score/lives/bombs/power/graze/cherry.
...
- Player data read straight from the original `.sht` files: movement
  ...
  windows (Reimu 15 / Marisa 8 / Sakuya 6 frames), 30-frame shot cycle.
...
- Full BGM set (`th07_01`–`th07_19` / `13b`) loops gaplessly using
  sample-exact loop points from `thbgm.fmt`; sound effects are the
  original WAVs.

> Fidelity work is ongoing: some late-stage boss phases, Stage 4 multi-slot
> sister choreography, a few bullet-effect ids, and background bank
> switching are still incomplete — see Known gaps below. Feedback welcome.
...
- No frame-by-frame replay comparison yet, so frame-level parity is not
  proven.
- The registry of known approximations lives in [AGENTS.md §7](AGENTS.md)
  (focused option orbit rate, bomb presentation, some HUD placements,
  spell-declaration presentation, the bottom-edge “Enemy” marker, …).
- Stage 4 multi-slot sister choreography, some late-stage boss
  death-callback stalls, bullet-effect ids 17/18 (seed detonation) and
  7/8 (laser bounce), and STD op 29 background-bank switching are still
  incomplete.
- Extra / Phantasm starting bombs/power follow community convention
  (PROBABLE, not exe-verified).

### Buy me some tokens (optional)

If this project was useful and you feel like covering a coffee or a bit of
API spend, you can send a little **USDT (ERC-20)** to the address below on
any EVM chain. Entirely optional — any amount is appreciated, and nothing
is gated behind it:
